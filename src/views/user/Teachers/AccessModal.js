...
const AccessModal = ({ open, onClose, user, roles = [] }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (Array.isArray(roles)) {
        userRoleNames = roles.map(r => typeof r === 'string' ? r.trim() : (r.roleName || r.name || r.title))
      } else if (typeof roles === 'string') {
        userRoleNames = roles.split(',').map(r => r.trim())
      }
