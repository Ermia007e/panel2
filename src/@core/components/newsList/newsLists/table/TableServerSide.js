...
import { serverSideColumns } from '../../common/formatDate/data'
...
const DataTableServerSide = ({ isActiveFilter, goToAddNews, onCountUpdate }) => {
  const [currentPage, setCurrentPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [searchValue, setSearchValue] = useState('');
  const [sortingCol, setSortingCol] = useState('updateDate');
  ...
  const { data: newsList, isLoading } = useQuery({
    ...
    onSuccess: (data) => {
      if (data) {
        onCountUpdate({
          active: isActiveFilter ? data.totalCount : 0,
          inactive: !isActiveFilter ? data.totalCount : 0
        });
      }
    }
  });
    ...
    <Card>
      <CardHeader className='border-bottom'>
        ...
        <Button
          onClick={goToAddNews}
          color="primary" 
          style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', whiteSpace: 'nowrap', height: '50px', color: '#fff', fontWeight: 400, fontSize: '1rem' }}>
          اضافه کردن خبر جدید
          <Plus/>
        </Button>
      </CardHeader>
      ...
      <div className='react-dataTable'>
        <DataTable
          ...
          sortServer
          columns={serverSideColumns(openedModalId, setOpenedModalId)}
          data={newsList?.news || []}
